...
class LocationRepository @Inject()(val dbConfigProvider: DatabaseConfigProvider, locationService: LocationService)(
      ...
      for {
        ...
        pairLocation.flatMap(pair => {
          myLocation.map(my => {
            UsersDistance(
              pair.userId,
              locationService.calcDistance(my.latitude, my.longitude, pair.latitude, pair.longitude)
            )
          })
        })
      }
