...
//    import com.github.tototoshi.slick.MySQLJodaSupport._
  ...
  final case class ShopsRow(shopId: Option[Int] = None,
                            shopName: String,
                            shopUrl: String,
                            shopLat: scala.math.BigDecimal,
                            shopLng: scala.math.BigDecimal)

  /** GetResult implicit for fetching ShopsRow objects using plain SQL queries */
  implicit def GetResultShopsRow(implicit e0: GR[Option[Int]],
                                 e1: GR[String],
                                 e2: GR[scala.math.BigDecimal]): GR[ShopsRow] = GR { prs =>
    import prs._
  ...
  class Shops(_tableTag: Tag) extends profile.api.Table[ShopsRow](_tableTag, "SHOPS") {
    ...
    /** Maps whole row to an option. Useful for outer joins. */
    def ? =
      (Rep.Some(shopId), Rep.Some(shopName), Rep.Some(shopUrl), Rep.Some(shopLat), Rep.Some(shopLng)).shaped.<>({ r =>
        import r._; _1.map(_ => ShopsRow.tupled((_1, _2.get, _3.get, _4.get, _5.get)))
      }, (_: Any) => throw new Exception("Inserting into ? projection not supported."))
