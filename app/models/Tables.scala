...
//    import com.github.tototoshi.slick.MySQLJodaSupport._
    ...
    lazy val schema: profile.SchemaDescription = Array(LoginStatuses.schema, MatchRelations.schema, Messages.schema, Shops.schema, Users.schema, UsersLocation.schema).reduceLeft(_ ++ _)
    ...
    class LoginStatuses(_tableTag: Tag) extends profile.api.Table[LoginStatusesRow](_tableTag, "LOGIN_STATUSES") {
        ...
        def ? = (Rep.Some(userId), Rep.Some(loginDatetime)).shaped.<>({ r => import r._; _1.map(_ => LoginStatusesRow.tupled((_1.get, _2.get))) }, (_: Any) => throw new Exception("Inserting into ? projection not supported."))
        ...
        lazy val usersFk = foreignKey("FK_LOGIN_STATUS_USERS", userId, Users)(r => r.userId, onUpdate = ForeignKeyAction.NoAction, onDelete = ForeignKeyAction.NoAction)
    }
    ...
    implicit def GetResultMatchRelationsRow(implicit e0: GR[Int], e1: GR[java.sql.Timestamp]): GR[MatchRelationsRow] = GR {
        prs =>
            import prs._
    ...
    class MatchRelations(_tableTag: Tag) extends profile.api.Table[MatchRelationsRow](_tableTag, "MATCH_RELATIONS") {
        ...
        /** Maps whole row to an option. Useful for outer joins. */
        def ? = (Rep.Some(userId), Rep.Some(partnerId), Rep.Some(matchState), Rep.Some(selectedDatetime)).shaped.<>({ r => import r._; _1.map(_ => MatchRelationsRow.tupled((_1.get, _2.get, _3.get, _4.get))) }, (_: Any) => throw new Exception("Inserting into ? projection not supported."))
        ...
        lazy val usersFk = foreignKey("FK_MATCH_USERS", userId, Users)(r => r.userId, onUpdate = ForeignKeyAction.NoAction, onDelete = ForeignKeyAction.NoAction)
    ...
    class Messages(_tableTag: Tag) extends profile.api.Table[MessagesRow](_tableTag, "MESSAGES") {
        ...
        /** Maps whole row to an option. Useful for outer joins. */
        def ? = (Rep.Some(userId), Rep.Some(partnerId), Rep.Some(message), Rep.Some(sendDatetime)).shaped.<>({ r => import r._; _1.map(_ => MessagesRow.tupled((_1.get, _2.get, _3.get, _4.get))) }, (_: Any) => throw new Exception("Inserting into ? projection not supported."))
        ...
        lazy val usersFk = foreignKey("FK_MESSAGES_USERS", userId, Users)(r => r.userId, onUpdate = ForeignKeyAction.NoAction, onDelete = ForeignKeyAction.NoAction)
    }
    ...
    /** Entity class storing rows of table Shops
      ...
      * @param shopLat  Database column SHOP_LAT SqlType(DECIMAL)
      * @param shopLng  Database column SHOP_LNG SqlType(DECIMAL) */
    final case class ShopsRow(shopId: Option[Int] = None, shopName: String, shopUrl: String, shopLat: scala.math.BigDecimal, shopLng: scala.math.BigDecimal)

    /** GetResult implicit for fetching ShopsRow objects using plain SQL queries */
    implicit def GetResultShopsRow(implicit e0: GR[Option[Int]], e1: GR[String], e2: GR[scala.math.BigDecimal]): GR[ShopsRow] = GR {
        prs =>
            import prs._
            ShopsRow.tupled((<<?[Int], <<[String], <<[String], <<[scala.math.BigDecimal], <<[scala.math.BigDecimal]))
    }
    ...
    class Shops(_tableTag: Tag) extends profile.api.Table[ShopsRow](_tableTag, "SHOPS") {
        def * = (Rep.Some(shopId), shopName, shopUrl, shopLat, shopLng) <> (ShopsRow.tupled, ShopsRow.unapply)

        /** Maps whole row to an option. Useful for outer joins. */
        def ? = (Rep.Some(shopId), Rep.Some(shopName), Rep.Some(shopUrl), Rep.Some(shopLat), Rep.Some(shopLng)).shaped.<>({ r => import r._; _1.map(_ => ShopsRow.tupled((_1, _2.get, _3.get, _4.get, _5.get))) }, (_: Any) => throw new Exception("Inserting into ? projection not supported."))
        ...
        /** Database column SHOP_LAT SqlType(DECIMAL) */
        val shopLat: Rep[scala.math.BigDecimal] = column[scala.math.BigDecimal]("SHOP_LAT")
        /** Database column SHOP_LNG SqlType(DECIMAL) */
        val shopLng: Rep[scala.math.BigDecimal] = column[scala.math.BigDecimal]("SHOP_LNG")
    }
    ...
    final case class UsersRow(userId: Option[Int] = None, userName: String, sex: Int, birthday: java.sql.Date, profile: Option[String], createdAt: java.sql.Date, mailAddress: String, password: String, profileImage: String)

    /** GetResult implicit for fetching UsersRow objects using plain SQL queries */
    implicit def GetResultUsersRow(implicit e0: GR[Option[Int]], e1: GR[String], e2: GR[Int], e3: GR[java.sql.Date], e4: GR[Option[String]]): GR[UsersRow] = GR {
        prs =>
            import prs._
            UsersRow.tupled((<<?[Int], <<[String], <<[Int], <<[java.sql.Date], <<?[String], <<[java.sql.Date], <<[String], <<[String], <<[String]))
    }

    /** Table description of table USERS. Objects of this class serve as prototypes for rows in queries. */
    class Users(_tableTag: Tag) extends profile.api.Table[UsersRow](_tableTag, "USERS") {
        def * = (Rep.Some(userId), userName, sex, birthday, profile, createdAt, mailAddress, password, profileImage) <> (UsersRow.tupled, UsersRow.unapply)

        /** Maps whole row to an option. Useful for outer joins. */
        def ? = (Rep.Some(userId), Rep.Some(userName), Rep.Some(sex), Rep.Some(birthday), profile, Rep.Some(createdAt), Rep.Some(mailAddress), Rep.Some(password), Rep.Some(profileImage)).shaped.<>({ r => import r._; _1.map(_ => UsersRow.tupled((_1, _2.get, _3.get, _4.get, _5, _6.get, _7.get, _8.get, _9.get))) }, (_: Any) => throw new Exception("Inserting into ? projection not supported."))
    ...
    implicit def GetResultUsersLocationRow(implicit e0: GR[Int], e1: GR[scala.math.BigDecimal]): GR[UsersLocationRow] = GR {
        prs =>
            import prs._
    ...
    class UsersLocation(_tableTag: Tag) extends profile.api.Table[UsersLocationRow](_tableTag, "USERS_LOCATION") {
        ...
        /** Maps whole row to an option. Useful for outer joins. */
        def ? = (Rep.Some(userId), Rep.Some(latitude), Rep.Some(longitude)).shaped.<>({ r => import r._; _1.map(_ => UsersLocationRow.tupled((_1.get, _2.get, _3.get))) }, (_: Any) => throw new Exception("Inserting into ? projection not supported."))
        ...
        lazy val usersFk = foreignKey("FK_USER_LOCATION_USERS", userId, Users)(r => r.userId, onUpdate = ForeignKeyAction.NoAction, onDelete = ForeignKeyAction.NoAction)
    }
