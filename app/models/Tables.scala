...
trait Tables {
    ...
    implicit def GetResultShopsRow(implicit e0: GR[Option[Int]], e1: GR[String]): GR[ShopsRow] = GR {
        prs =>
            import prs._
    ...
    class Shops(_tableTag: Tag) extends profile.api.Table[ShopsRow](_tableTag, "SHOPS") {
        ...
        /** Maps whole row to an option. Useful for outer joins. */
        def ? = (Rep.Some(shopId), Rep.Some(shopName), Rep.Some(shopUrl)).shaped.<>({ r => import r._; _1.map(_ => ShopsRow.tupled((_1, _2.get, _3.get))) }, (_: Any) => throw new Exception("Inserting into ? projection not supported."))
